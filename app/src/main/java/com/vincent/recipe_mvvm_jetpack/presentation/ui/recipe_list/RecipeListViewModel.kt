...
class RecipeListViewModel @Inject constructor(
    ...
    private suspend fun nextPage() {
        // prevent duplicated events due to recompose happening to quickly(getting to much response)
        if ((recipeListScrollPosition + 1) >= (page.value * PAGE_SIZE)) {
            loading.value = true // Start Loading

            incrementPage() // Increase the page number
            ...
            if (page.value > 1) {
                ...
                for (recipe in result) {
                    Log.d(TAG, "Page ${page.value} : $recipe")
                }
            ...
            Log.d(TAG, "refresh too often, refuse to request.")
        }
    }
